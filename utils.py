...
# used in pre_train.py
def load_data4pretrain(dataname='CiteSeer', num_parts=None, tasks_type='node_level'):
    graph = torch.load(f"./dataset/{dataname}/data.pt")
    dataset = [graph,graph]
    #print('xxxx',graph.data)
    #data = dataset.data
...
def load_state_dict_from_path(model, path, device):
    ...
    if 'state_dict' in checkpoint.keys():
        ...
        try:
            print(f'reloaded with strict=False \n')
            incompatible = model.load_state_dict(checkpoint, strict=False)
            if incompatible is not None:
                  print(incompatible)
                  print(f'Num matches: {len([k for k in model.state_dict() if k in checkpoint])}\n'
                        f'Num missing: {len(incompatible.missing_keys)} \n'
                        f'Num unexpected: {len(incompatible.unexpected_keys)}')
        except RuntimeError as e:
            print(f'cannot reload model')
